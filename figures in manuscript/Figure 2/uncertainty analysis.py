...
for _, row in df_long.iterrows():
    ...
    ax.axis("off")
    ax.set_ylim(0, max(y_vals) * 1.2)  # 预留顶部空间放文字

    for j in range(len(x_vals) - 1):
        mid_val = (x_vals[j] + x_vals[j + 1]) / 2
        clipped_val = min(max(mid_val, global_min), color_cap)
        col_idx = (clipped_val - global_min) / (color_cap - global_min)
        col_idx = np.clip(col_idx, 0, 1)
        ax.fill_between([x_vals[j], x_vals[j + 1]], 0, [y_vals[j + 1], y_vals[j]],
                        color=cmap(int(col_idx * 99)))

    # 标注 "列名 - 行名"
    ax.text(np.mean(x_vals), max(y_vals) * 1.05,
            f"{col_name} - {row_name}",
            ha="center", va="bottom", fontsize=9, fontweight="bold", color="black")

    plt.subplots_adjust(left=0, right=1, top=1, bottom=0)
    fig.patch.set_alpha(0.0)
    safe_name = "".join(c if c.isalnum() or c in "-._" else "_" for c in case_name)[:100]
    fig.savefig(os.path.join(plot_dir, f"distribution_{safe_name}.png"), transparent=True)
    plt.close(fig)
